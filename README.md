...
- **Deduplication** — each show appears once, with all of its airings (channel + time) collected.
- **Filtering** — by country, day range, genre, and minimum rating.
...
|-------------|--------|---------|-----------------------------------------------|
| `country`   | string | `GB`    | ISO 3166-1 country code (e.g. `GB`, `US`, `AU`) |
...
```bash
curl "http://localhost:8080/api/recommendations?country=GB&days=3&genre=Drama&minRating=7"
```

Each item includes the title, type, genres, summary, image, composite score, the individual
rating sources behind it, every airing (channel + start time + episode) in the window, and a
plain-language `why`.
...
|-------------------------------|---------------------------|--------------------------------------|
| `tv.defaults.country`         | `GB`                      | Default country code                 |
...
2. Group airings by show (deduplicate); keep the soonest-first list of airings.
...
5. Apply genre / minimum-rating filters, sort by score (ties broken by soonest airing), and cap
